...
class SantosVega(Personaje):
    ...
    def hablar(self, texto: str, interacti=True):
        if renpy and self._personalizado:
            self._personalizado(f"{texto}",interact=interacti)
        elif renpy:
            renpy.exports.say(self.nombre, f"{texto}",  interact=interacti)  #polimorfismo en el método hablar de ésta subclase
        else:
